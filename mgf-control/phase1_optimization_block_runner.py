"""Phase 1 Optimization Block Runner - divides 625 combos into 5 blocks of 125 each
...
def run_block(block_num: int, smooth_h_values: list, phase_a_rows: list, phase_b_rows: list):
    ...
    print(f"\n{'='*80}")
    print(f"PHASE 1 OPTIMIZATION - BLOCK {block_num}")
    print(f"{'='*80}")
    ...
    block_filename = f"phase1_optimization_block_{block_num}_log.csv"
    ...
    # Export block top 10
    top_10 = results[:10]
    block_top_filename = f"phase1_optimization_block_{block_num}_top10.csv"
    ...
    if best_params:
        ...
        block_best_filename = f"phase1_optimization_block_{block_num}_best_params.json"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python phase1_optimization_block_runner.py <block_number>")
